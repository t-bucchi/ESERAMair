...
mDNS が使えない環境(別のサブネットなど)ではこの IPアドレスを使用します。
...
お使いのブラウザで、`http://eseram.local/` もしくは `http://IPアドレス/` を開いてください。
ESERAMair は mDNS に対応しているので、同一のサブネット内であれば `eseram.local` でアクセスすることが可能です。

> [!NOTE]
> `eseram.local` は [USBCLI.mdの hostname コマンド](USBCLI.md#hostname)で変更することも可能です。複数のESERAMairを使い分ける時には別のホスト名を設定してください。
...
「Clear the SRAM before uploading the ROM」にチェックを入れると、アップロード前にSRAMをクリアします。基本的にチェックを入れてください。

すでにESERAMair内にファイルが格納されていて、ファイルを消さずにNextorROMのみを更新したいときはチェックを外してください。
...
> [!NOTE]
> ディスクの容量は 768kB です。
> 2DD のフロッピーディスクよりも僅かに大きい容量となります。
>
> 1024kBのうち、Nextor kernel と ESERAMair ドライバで 256kB を消費します。


## ファイルの upload / download / rename / delete
...
> [!NOTE]
> 上の画像の赤枠の領域にファイルをドラッグアンドドロップすることでもアップロードすることができます。
...
> [!NOTE]
> MSX-DOS2 を使用中にファイルのアップロードを行うと、カレントディレクトリが ¥ に戻ってしまいます。
> Webからの書き込みを MSX に認識させるために、一度ディスクを取り出した状態にするための制限です。

### ファイルのダウンロード

ファイルをダウンロードするには、ダウンロードしたいファイルの左端のチェックボックスにチェックを入れてください。

ヘッダのチェックボックスを使うと、表示中の一覧を一括選択/一括解除できます。

<img width="600" src="image/file-download-rename.png">

チェックした状態で画面上部の「Download」ボタンで、選択中のファイルやディレクトリをPCへダウンロードできます。

- ファイルを1つだけ選択した場合:
  - そのファイルをそのままダウンロードします
- 複数ファイルを選択した場合:
  - `.zip` 形式でまとめてダウンロードします
- ディレクトリを選択した場合:
  - 配下のファイルも含めて `.zip` 形式でダウンロードします

### ファイルやディレクトリの削除

画面上部の「Delete」ボタンで、チェックしたファイルやディレクトリを削除できます。

<img width="600" src="image/file-download-rename.png">

ディレクトリを選択した場合は、配下のファイルやサブディレクトリもまとめて削除されます。

> [!WARNING]
> 削除したファイルやディレクトリは元に戻せません。特にディレクトリを削除する場合は、配下も含めて消える点に注意してください。

### ファイル名の変更

ファイル一覧の右端にある Rename アイコンを押すと、ファイル名やディレクトリ名を変更できます。

- 新しい名前は MSX の 8.3 形式で入力してください
- 今のところ、ひらがな半角カタカナなどは入力できません
- すでに同じ名前のファイルやディレクトリがある場合は変更できません

入力内容に問題があると、入力欄が赤または黄色で表示され、「Rename」ボタンは押せません。
